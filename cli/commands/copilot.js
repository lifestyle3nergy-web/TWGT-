  ...
  try {
    switch (action) {
      ...
      default:
        ...
        console.log('Available cloud actions: sync, pull, status, config, logs, deploy');
        process.exit(1);
    }
  } catch (err) {
...
async function initCopilot(_opts) {
  ...
  try {
    ...
    require('fs').writeFileSync('.copilot/config.json', JSON.stringify(config, null, 2));
...
async function statusCopilot(_opts) {
  ...
  try {
    ...
    if (fs.existsSync('.copilot/config.json')) {
      const config = JSON.parse(fs.readFileSync('.copilot/config.json', 'utf-8'));
