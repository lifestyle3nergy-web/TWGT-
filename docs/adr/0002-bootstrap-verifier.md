...
The verifier must reject missing, malformed, placeholder, or unverifiable evidence. Tests must exercise negative cases for trust material and digest failures. A positive cryptographic test may only be added after the authoritative signature contract exists; test-only trust material must remain ephemeral.
