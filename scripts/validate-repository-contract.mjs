...
const topLevel = new Set(
  files.map((file) => file.split('/')[0]).filter(Boolean),
);
...
if (changed.length > 0) {
  const adds = run(['diff', '--name-status', `origin/${baseRef}...HEAD`])
    .split('\n')
    .filter(Boolean)
    .map((line) => line.split('\t'));

  for (const [status, path] of adds) {
    if (status === 'A' && !tracked.has(path)) continue;
    if (status === 'A' && path && path.split('/').length === 1 && !baselineTopLevel.has(path)) {
      errors.push(`New top-level file requires explicit contract review: ${path}`);
    }
  }
