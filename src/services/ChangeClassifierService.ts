...
const RULES: ReadonlyArray<{ category: ChangeCategory; patterns: RegExp[] }> = [
  {
    ...
    patterns: [
      ...
      /(^|\/)Dockerfile$/,
      /(^|\/)scripts\//,
      /(^|\/)(compose\.ya?ml|docker-compose\.ya?ml)$/i,
      /(^|\/)(infra|infrastructure|deploy|deployment|terraform|k8s|kubernetes)\//i,
    ],
  },
  {
    category: 'security',
  ...
  { category: 'routing', patterns: [/(^|\/)(plugins?|models?|routing)\//, /gateway/i] },
  {
    ...
    patterns: [/(^|\/)(telemetry|observability|logging|metrics)\//, /opentelemetry/i],
  },
  {
    category: 'repository-structure',
    patterns: [/^(package|tsconfig|vitest)\..+$/, /workspace/i, /turbo\.json$/],
  },
  {
    category: 'runtime',
    patterns: [
