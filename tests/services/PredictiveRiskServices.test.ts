...
import {
  ...
  type RawEvidence,
} from '../../src/services/EvidenceCollectorService';
...
} from '../../src/contracts/ExecutionEnvelope';

const fixturePath = fileURLToPath(
  new URL('../fixtures/predictive-risk/security-failure.json', import.meta.url),
);
...
describe('predictive risk foundation', () => {
  ...
  it('redacts evidence and rejects a demonstrated scan failure', () => {
    const evidence = new EvidenceCollectorService().collect(fixture.evidence, [
      'commit',
      'check',
      'test',
      'scan',
    ...
    expect(risk.uncertainty).toEqual(
      expect.arrayContaining([
        expect.stringContaining('missing check'),
  ...
  it('prevents missing evidence from silently passing', () => {
    ...
    const envelope: ExecutionEnvelope = {
      ...
      evidence,
      risk: { score: risk.score, reasons: risk.reasons, uncertainty: risk.uncertainty },
      decision: 'PASS',
  ...
  it.each([
    ...
    expect(
      new PolicyEvaluatorService().evaluate(
  ...
  it('rejects policies that relax approval or production boundaries', () => {
    ...
    expect(
      evaluator.evaluate(risk, evidence, {
