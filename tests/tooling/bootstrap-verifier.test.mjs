...
test('rejects invalid schema field types and empty identity values', () => {
  const invalidValues = [
    { ...base, schemaVersion: {} },
    { ...base, nucleusId: '' },
    { ...base, nucleusTag: '' },
    { ...base, nucleusVersion: '' },
    { ...base, signature: { ...base.signature, extra: 'undeclared' } },
  ];
  for (const evidence of invalidValues) {
    const result = runVerifier(evidence);
    assert.notEqual(result.status, 0);
  }
});

test('rejects digest artifacts that are symbolic links', () => {
  const { publicKey } = generateKeyPairSync('ed25519');
